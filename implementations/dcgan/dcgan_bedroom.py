...
# light on how and why this models works. But don’t worry, no prior
...
# ~~~~~~~~~~~~~~
#
# GANs are a framework for teaching a DL models to capture the training
...
# *discriminator*. The job of the generator is to spawn ‘fake’ mnist_images that
# look like the training mnist_images. The job of the discriminator is to look
...
# detective and correctly classify the real and fake mnist_images. The
...
# generator. Here, since we are dealing with mnist_images the input to
...
# loss functions, and how to initialize the models weights, all of which
...
# -  **image_size** - the spatial size of the mnist_images used for training.
#    This implementation defaults to 64x64. If another size is desired,
#    the structures of D and G must be changed. See
#    `here <https://github.com/pytorch/examples/issues/70>`__ for more
#    details
# -  **nc** - number of color channels in the input mnist_images. For color
#    mnist_images this is 3
...
# Spatial size of training mnist_images. All mnist_images will be resized to this
# size using a transformer.
image_size = 64

# Number of channels in the training mnist_images. For color mnist_images this is 3
nc = 3
...
# Plot some training mnist_images
...
# ~~~~~~~~~~~~~~~~~~~~~
#
# From the DCGAN paper, the authors specify that all models weights shall
# be randomly initialized from a Normal distribution with mean=0,
# stdev=0.02. The ``weights_init`` function takes an initialized models as
...
# (:math:`z`) to data-space. Since our data are mnist_images, converting
# :math:`z` to data-space means ultimately creating a RGB image with the
# same size as the training mnist_images (i.e. 3x64x64). In practice, this is
...
# channels in the output image (set to 3 for RGB mnist_images). Below is the
...
# function. Check out the printed models to see how the generator object is
# structured.
#
...
# ``weights_init`` function, and print the models’s structure.
#
...
# mnist_images form out of the noise.
#
...
# mnist_images, and also adjust G’s objective function to maximize
...
#
# Finally, lets take a look at some real mnist_images and fake mnist_images side by
# side.
#

# Grab a batch of real mnist_images from the dataloader
real_batch = next(iter(dataloader))

# Plot the real mnist_images
...
# Plot the fake mnist_images from the last epoch
plt.subplot(1, 2, 2)
plt.axis("off")
...
# -  Modify this models to take a different dataset and possibly change the
#    size of the mnist_images and the models architecture
