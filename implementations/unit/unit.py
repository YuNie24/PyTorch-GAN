...
parser.add_argument("--checkpoint_interval", type=int, default=-1, help="interval between saving models checkpoints")
...
os.makedirs("mnist_images/%s" % opt.dataset_name, exist_ok=True)
...
# Loss weights
lambda_0 = 10  # GAN
lambda_1 = 0.1  # KL (encoded mnist_images)
lambda_2 = 100  # ID pixel-wise
lambda_3 = 0.1  # KL (encoded translated mnist_images)
...
def sample_images(batches_done):
    ...
    save_image(img_sample, "mnist_images/%s/%s.png" % (opt.dataset_name, batches_done), nrow=5, normalize=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        # Reconstruct mnist_images
        recon_X1 = G1(Z1)
        recon_X2 = G2(Z2)

        # Translate mnist_images
        fake_X1 = G1(Z2)
        fake_X2 = G2(Z1)
