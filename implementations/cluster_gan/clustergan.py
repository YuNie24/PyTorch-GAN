...
class Reshape(nn.Module):
    """
    Class for performing a reshape as a layer in a sequential models.
    """
...
class Generator_CNN(nn.Module):
    """
    CNN to models the generator of a ClusterGAN
...
class Encoder_CNN(nn.Module):
    """
    CNN to models the encoder of a ClusterGAN
...
class Discriminator_CNN(nn.Module):
    """
    CNN to models the discriminator of a ClusterGAN
...
for epoch in range(n_epochs):
    for i, (imgs, itruth_label) in enumerate(dataloader):
        ...
        # Generate a batch of mnist_images
        ...
        if (i % n_skip_iter == 0):
            # Encode the generated mnist_images
    ...
    save_image(reg_imgs.data[:n_samp],
               'mnist_images/cycle_reg_%06i.png' %(epoch),
               nrow=n_sqrt_samp, normalize=True)
    save_image(gen_imgs_samp.data[:n_samp],
               'mnist_images/gen_%06i.png' %(epoch),
               nrow=n_sqrt_samp, normalize=True)
    ...
    save_image(stack_imgs,
               'mnist_images/gen_classes_%06i.png' %(epoch),
