...
def sample_images(batches_done):
    ...
    for i in range(10):
        ...
        # Concatenate mnist_images by width
        gen_imgs = torch.cat([x for x in gen_imgs.data], -1)
        img_sample = torch.cat((img.data, gen_imgs), -1)
        # Add as row to generated samples
        img_samples = img_sample if img_samples is None else torch.cat((img_samples, img_sample), -2)

    save_image(img_samples.view(1, *img_samples.shape), "mnist_images/%s.png" % batches_done, normalize=True)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, (imgs, labels) in enumerate(dataloader):
        ...
        # Generate fake batch of mnist_images
